...
O.k. That's much better. But if we really want to know the relationship between sample size and the number of eQTL, then let's plot the number of sample of each tissue type on the same plot.
...
p <- p + geom_point(aes(x=tissue, y=N*150)) + 
  scale_y_continuous(sec.axis = sec_axis(~./150, name = "Sample Size"))
print(p)
```

...
## The `ggplot` function
Any `ggplot` plot type  will start with the `ggplot` function. It is used to define the dataset and the variables that will define the x and y axis (and other possible attributes). 
Aes is short of aesthetics, which defines the relationship between the variables in the dataframe and plot features.
...
You can also specify the number of bins (`bins=`), the actualy breaks for the bins (`breaks=`), and if the intervals are left or right closed (`closed=<left/right>`)

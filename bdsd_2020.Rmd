---
title: <center>'BDSD 2020\nIntroduction to ggplot2' </center>
output:
  html_document: 
    toc: true
...
First we will plot the number of eQTL as a function of tissue using a histogram. Since a non-NA cell indicates that it is an eQTL, all we have to do is sum the number of non-NA cells in a column (tissue) to count the number of eQTL.
...
`fill` specifies the color of the bars. Later we'll see that fill can be used in another way to assign color to an attribute of the dataset.

## Changing the look and feel of a plot using `theme`
There are a few things that we can do to make this plot look nicer.

1. I find the grey background unattractive. There are predefined themes that provide alternatives. The one I use often is `theme_minimal`. Others to look at include: `theme_gray`, `theme_bw`, `theme_light`, `theme_dark`, `theme_classic`, `theme_void`. Others have created packages with other themes. You can also make your own.
...
One nice thing about ggplot is that additional plotting commands can be added to the  plot variable (`p`) . This allows conditional statement to help determine what will be contained in the plot.
...
To do this we have to do two things. First is to plot the sample size. We'll use geom_points to do this. The second is to scale the value of the number of samples, sincce the number of eQTL and number of samples is quite different. 
...
## `facet_grid`: Creating a matrix of plots to simply plotting multiple variables
...
print(p)
```
# Plotting similarity and `heatmaps`
...
Unfortunately, when you don't cluster the rows or columns it not only omits the cladogram, but it doesn't perform the clustering either. We can apply the same trick we did previously to reorder the rows and columns, however this time we have to literally reorder the rows and columns. 
...
2. scale_fill_gradient: Allows for a continuous gradient between two colors. 
...
3. scale_fill_gradient2: Allow for a continuous gradient between two sets of color pairs.
...
4. scale_fill_gradientn: Allow for a continuous gradient between multiple sets of color pairs.
...
5. scale_fill_brewer: Allow for easy integration of RColorBrewer palettes. Works on discrete scale only. 
